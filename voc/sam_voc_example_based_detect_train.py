        ...
        for img_idx, img_result in enumerate(self.per_image_results):
            ...
            for category in self.VOC_CLASSES:
                ...
                for missed in category_missed:
                    bbox_xyxy = missed['bbox']
                    # print("missed bbox_xyxy:", bbox_xyxy)
                    # bbox_xyxy_tensor = torch.tensor(bbox_xyxy).view(-1, 4)
                    # print("missed bbox_xyxy_tensor:", bbox_xyxy_tensor)
                    #
                    # bbox_cxcywh = box_xywh_to_cxcywh(bbox_xyxy_tensor)
                    # print("missed bbox_cxcywh:", bbox_cxcywh)
                    ...
                    # norm_box = normalize_bbox(bbox_cxcywh, width, height).flatten().tolist()
                    # print("missed norm_box:", norm_box)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--per-image-results',
        type=str,
        default='./voc_eval_results_v2_train/per_image_results.json',
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='./voc_refinement_results_train_v2',
