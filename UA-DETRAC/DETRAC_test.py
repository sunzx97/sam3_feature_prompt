...
# sys.path.insert(0, '/home/sun/data/coding/object_detection_based_on_context_learning/SAM_based/sam3-main')
...
class DETRACTester:
    ...
    #         scores = results['scores'].cpu().numpy() if 'scores' in results else np.ones(len(boxes))
    #
    ...
    def create_xml_for_video(self, video_name, frame_results, output_path):
        ...
        Args:
            video_name: 视频名称
            frame_results: dict, key=frame_num, value=dict with keys:
                - 'matched_results': list of matched predictions
                - 'unmatched_gt_ids': list of missed GT IDs
                - 'all_gt_ids': list of all GT IDs in this frame
                - 'gt_id_map': dict mapping gt_id to gt_info
        """
    ...
    def test_dataset(self, images_root, gt_xml_root, output_root):
        ...
        for video_folder in tqdm(video_folders, desc="处理视频"):
            ...
            gt_data = self.load_gt_annotations(gt_xml_path)
            frame_results = {}
            ...
            for img_path in tqdm(image_files, desc=f"  {video_name}", leave=False):
                ...
                try:
                    predictions = self.detect_frame(img_path)
...
def main():
    ...
    IMAGES_ROOT = r"E:/user/szx/sam/UA-DETRAC/DETRAC_test"
    GT_XML_ROOT = r"E:/user/szx/sam/UA-DETRAC/DETRAC-Test-Annotations-XML"
